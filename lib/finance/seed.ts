...
export function createInitialFinanceState(): FinanceState {
  return {
    ...
    accountSummaries: financeSeed.accountSummaries.map((summary) => ({ ...summary })),
    categories: financeSeed.categories.map((category) => ({ ...category })),
    counterparties: financeSeed.counterparties.map((counterparty) => ({ ...counterparty })),
    transactions: financeSeed.transactions.map((transaction) => ({ ...transaction })),
    budgets: financeSeed.budgets.map((budget) => ({ ...budget })),
    budgetSummaries: financeSeed.budgetSummaries.map((summary) => ({ ...summary })),
