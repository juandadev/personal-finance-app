...
export const config = {
  matcher: [
    {
      source:
        "/((?!api/auth|api/webhooks/neon-auth|api/cron|_next/static|_next/image|favicon.ico|images|login|sign-up|verify-email|privacidad).*)",
      ...
      // enforce its own auth check (see requireUserId() in lib/auth/session.ts)
